import React, { useState, useEffect } from 'react';
import { Link } from 'react-router-dom';
import { PostFell } from '../../../utils/animalController';

const CreateFell = () => {
    const [fell, setFell] = useState('');

    const handleFell = () => {
        if (!fell) {
            alert('Por favor, preencha todos os campos!');
        } else {
            PostFell(fell);
        }
    };

    return (
        <>
            <h1 className="text-4xl font-black text-white text-center mt-16">Pelo</h1>
            <div className="relative z-0 w-full mb-6 group">
                <input
                    type="text"
                    value={fell}
                    onChange={(e) => setFell(e.target.value)}
                    className="block py-2.5 px-0 w-full text-sm text-white bg-transparent border-0 border-b-2 border-gray-300 appearance-none focus:outline-none focus:ring-0 focus:border-blue-600 peer"
                    placeholder=" "
                    required
                />
                <label
                    htmlFor="pelo"
                    className="peer-focus:font-medium absolute text-sm text-gray-500 duration-300 transform -translate-y-6 scale-75 top-3 -z-10 origin-[0] peer-focus:left-0 peer-focus:text-blue-600 peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6"
                >
                    Pelo
                </label>
            </div>
            <Link
                to="../admin/dashboard"
                className="text-white bg-green-700 hover:bg-green-800 focus:ring-4 focus:outline-none focus:ring-green-300 font-medium rounded-lg text-sm w-full sm:w-auto px-5 py-2.5 text-center"
            >
                Voltar
            </Link>
            <Link
                type="submit"
                onClick={handleFell}
                className="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm w-full sm:w-auto px-5 py-2.5 text-center ml-2"
            >
                Salvar
            </Link>
        </>
    );
};

export default CreateFell;
